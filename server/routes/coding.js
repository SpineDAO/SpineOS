...
codingRouter.post('/analyze', requireAuth, async (req, res) => {
  ...
  try {
    const result = await analyzeOpNote(opNoteText, payerName)
...
codingRouter.post('/appeal', requireAuth, async (req, res) => {
  ...
  try {
    const letter = await generateAppealLetter(code, codeDescription, denialReason, payerName, opNoteExcerpt)
    res.json({ letter })
  } catch (err) {
